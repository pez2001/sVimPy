...
//move these as definitions into constructor
// PORTB pin numbers
//
// arduino D10
#define SS    2
...
byte mmc::spiTransferByte(byte data)
{
  // send the given data
  SPDR = data;
...
uint32_t mmc::spiTransferLong(const uint32_t data)
{
...
char mmc::sdResponse(byte expected)
{
...
char mmc::sdWaitWriteFinish(void)
{
...
void mmc::deselectCard(void) {
  // Send 8 clock cycles
  SPI_SS_HIGH();
  spiTransferByte(0xff);
}

byte mmc::crc7update(byte crc, const byte data) {
  byte i;
  bool bit;
  byte c;
