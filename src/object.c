...
*/
int
IsIntObject (object * obj)
{
  return (obj->type == TYPE_INT);
}

int
IsStringObject (object * obj)
{
  return (obj->type == TYPE_STRING);
}

int
IsUnicodeObject (object * obj)
{
  return (obj->type == TYPE_UNICODE);
}

int
IsCodeObject (object * obj)
{
  return (obj->type == TYPE_CODE);
}

int
IsTupleObject (object * obj)
{
...
string_object *
AsStringObject (object * obj)
{
  return ((string_object *) obj->ptr);
}

code_object *
AsCodeObject (object * obj)
{
  return ((code_object *) obj->ptr);
}

caller_object *
AsCallerObject (object * obj)
{
  return ((caller_object *) obj->ptr);
}

function_object *
AsFunctionObject (object * obj)
{
  return ((function_object *) obj->ptr);
}

tuple_object *
AsTupleObject (object * obj)
{
  ...
  switch (obj->type)
	  ...
	  if (((tuple_object *) obj->ptr)->num > 0)
	      ...
	      for (int i = 0; i < ((tuple_object *) obj->ptr)->num; i++)
		{
		  if ((((tuple_object *) obj->ptr)->items[i]) != NULL
		      && !(((tuple_object *) obj->ptr)->
			   items[i]->flags & OFLAG_ON_STACK))
		    {
	...
	case TYPE_CODE:
	  ...
	  objects_header_total -=
	    strlen (((code_object *) obj->ptr)->name) + 1;
...
*/
void
DumpObject (object * obj, int level)
{
  if (obj == NULL)
    return;
//printf("level:%d\n",level);
		  ...
		  if (((tuple_object *) obj->ptr)->items[i] != NULL)
		    {
		      ...
		      if ((((tuple_object *) obj->ptr)->
			   items[i]->flags & OFLAG_TUPLE_PTR) > 0)
			printf ("->");
...
object *
FindUnicodeTupleItem (object * tuple, char *name)
{
...
void
SetItem (object * tuple, int index, object * obj)
{
  if (tuple == NULL || tuple->type != TYPE_TUPLE)
    return;
...
void
DeleteItem (object * tuple, int index)
{
  if (tuple == NULL || tuple->type != TYPE_TUPLE)
    return;
  if (index >= ((tuple_object *) tuple->ptr)->num || index < 0)
...
object *
GetItem (object * tuple, int index)
{
  if (tuple == NULL || tuple->type != TYPE_TUPLE)
    return (NULL);
  if (index >= ((tuple_object *) tuple->ptr)->num || index < 0)
